...
<script language="Javascript">
  ...
  $(function () {
     ...
     * 禁用文本选中
     */
    $(window).bind('selectstart', function (evt) {
      evt.stopPropagation();
      evt.preventDefault();
      return false;
    });
    ...
    function reSizeFullPage() {
      ...
      var minusHeight = 320;
      var minusHeight2 = 160;
      if (window.location.href.indexOf('exam_pre.jsp') != -1) {
        minusHeight = 173;
        minusHeight2 = 113;
      }

      if (height < 660) { //小屏幕隐藏页脚
        $('.widget-body').css('min-height', (height - minusHeight));
        $('.footer').hide();
      } else { // 大屏幕显示页脚
        $('.widget-body').css('min-height', (height - minusHeight2 - footerHeight));
        $('.footer').show();
      }
    }
